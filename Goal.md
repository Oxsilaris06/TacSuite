...
>
> **STATUT (2026-08-12, commits locaux 749fe07/1191cca/f6c9946, rien poussé)** : ✅ **livrés** depuis la révision 2 — étape 0 (coupes §8), les **14 quick wins S** (U1-U14), **tranches M** (U15-U26, photo↔ping §4, U16/C1, C5, C8), et en plus ce jour : **fuite export photos OI corrigée** (exportArchive dumpait tout le store IndexedDB au lieu des images référencées ; orphelins nettoyés à la suppression de bloc ; `photoId` orphelin purgé côté PC-Tac aussi) et **parité carto OI/PC-Tac** (fond IGN, labels rues, toolbar/légende, roue de création, panneaux membre/véhicule, formes éditables, mesure, texte libre, photo↔ping — machine à gestes formes extraite en `src/shared/shape-gestures.ts`). Gate : typecheck 0, lint 0, vitest 1884/1884. **Restent** : structurel U27-U33, carte séparée différée (§5), quelques restes mineurs actés en §4/§7.
...
Restes mineurs actés : statut non propagé à la couleur du pin carte ; select lien otage rafraîchi à l'affichage de l'onglet.
~~pas de purge `pcTacPlanPins` à la suppression d'une photo~~ **corrigé 2026-08-12** (purge `photoId` des pins à la suppression de photo, cf. journal des exports).

### Carto OI — parité PC-Tac — ✅ LIVRÉE (2026-08-12, commits 1191cca/f6c9946)
Arbitrage Nico (2026-08-12) : parité ciblée + mesure/gestes/texte, entités PATRACDVR conservées, stratégie hybride. Fond IGN BD ORTHO + labels rues togglables/persistés ; toolbar 4 FABs + tiroir « Plus » + légende ; roue de création ping (clic/appui long) avec quick-place + panneaux membre/véhicule PATRACDVR ; roue options pin enrichie (verrou, copie coords MGRS+GPS) ; formes éditables (sélection/poignées/drag/resize/pinch) via `src/shared/shape-gestures.ts` (extraction de la machine à gestes formes PC-Tac, adaptateur mince, API intacte) ; précision de tracé (réticule) ; mesure (distance+azimut+anneaux d'engagement) ; texte libre (roue → `promptDialog`, persisté `cartography.texts`) ; photo↔ping complet (badge, panneau vignettes, viewer PhotoSwipe, orphelins nettoyés, auto-embarqué à l'export). Tuto 57 steps.
Restes connus : modale ping dormante (`#oi_carto_ping_modal` + `_openPingModal` conservés car `_armPinPlacement` encore appelé par `_renderPingLists`) ; badge cadenas sur marker OI non porté ; rendu textes en recreate complet (pas de réconciliation par signature) ; accumulation store OI possible si import sans « Photos HD » (purge non modifiée côté import, l'export filtré rend le point bénin).
