...
| 23b | Plan : ombrage LiDAR HD IGN (MNT/MNS/MNH) togglable | VERT | E2E (`window.PlanMap.lidarLayer`) + unitaire | AJOUT hors portage `planMap.js` : `Plan — ombrage LiDAR HD (#plan_btn_lidar) : cyclage MNT/MNS/MNH/aucun` — quatre clics parcourent `mnt → mns → mnh → null`, le FAB acquiert/perd `.active` et `pcTacPlanLidar` est effacée à l'extinction. Les 3 ressources WMTS Géoplateforme sans clé (`IGNF_LIDAR-HD_{MNT,MNS,MNH}_ELEVATION.ELEVATIONGRIDCOVERAGE.SHADOW`, TileMatrixSet `PM` = XYZ) sont déclarées MASQUÉES dans `RASTER_STYLE` : aucune tuile n'est requêtée tant qu'aucune n'est visible. Exclusivité des 3 couches, cyclage, persistance et restauration d'une clé corrompue : `tests/unit/pctac/pm-mapcore.test.ts` §6. L'ombrage actif part avec une AOI hors-ligne (`styleTileTemplates(extraSourceIds)`, `tests/unit/pctac/pm-geo.test.ts`). Accès réseau réel au service IGN NON exercé en CI — sonde dédiée : `node scripts/check-ign-lidar.mjs` |
| 23c | Plan : fond topo couleur Plan IGN v2 + courbes de niveau | VERT | E2E (`window.PlanMap.planIgnOn/contoursOn`) + unitaire | AJOUT hors portage `planMap.js` : l'IGN ne diffusant le LiDAR HD qu'en niveaux de gris, la couleur vient du fond. `Plan — fond Plan IGN et courbes de niveau (#plan_btn_topo, #plan_btn_contours)` : les deux bascules sont INDÉPENDANTES (éteindre le fond n'emporte pas les courbes) et persistées séparément (`pcTacPlanTopo`, `pcTacPlanContours`). Sources WMTS keyless `GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2` (licence Etalab 2.0) et `ELEVATION.CONTOUR.LINE`, déclarées masquées. Empilement contractuel vérifié en unitaire ET en navigateur réel : `satellite → ign-ortho → planign → ombrages LiDAR → contours` (couleur en bas, relief au milieu, lignes lisibles au-dessus). L'opacité de l'ombrage LiDAR SUIT le fond — 0,85 sur l'imagerie, 0,45 sur le Plan IGN — sans quoi l'ombrage masquerait les figurés de la carte. Seules les couches actives partent dans une AOI. Existence réelle de `ELEVATION.CONTOUR.LINE` NON exercée en CI (aucun accès réseau IGN) — à confirmer par `node scripts/check-ign-lidar.mjs`, qui sonde les 5 ressources et rapporte les zooms réellement servis |
