...
Source critères : `docs/recon-oi.md` §9 (« Checklist fonctionnelle de
non-régression — Générateur d'OI »). Statuts vérifiés 2026-08-01 contre
...
post-câblage P3B.C — 9 `TODO-CABLAGE` brouillon résolus par lecture
directe code câblé réel, cf. en-tête fichier) et par certification PDF
structurelle / comparaison visuelle pour items non exprimables en simple
assertion DOM.

## Run de référence (2026-08-01, mise à jour P3B.FIX reprise 2)

Cette section a été réécrite : version précédente décrivait protocole
(`--workers=2` manuel, `actionTimeout`/`expect.timeout` 2000ms) périmé depuis commits `b498e8b` (BLOQUANT R2 — `workers` figé à `1` **dans**`playwright.config.ts`, plus besoin de flag manuel) et `3b1cc30` (timeouts
relevés à 3000ms) — et affirmait `compare.mjs oi` **18/18 PASS** nonreproductible en l'état (non-déterminisme identifié et corrigé en reprise 2,
voir plus bas).
 ...
 **sans flag** — `playwright.config.ts` fixe désormais lui-même
 ...
 3 runs.- **PDF** : recette `.tacsuite-prep/oi-reference/recipe.md` rejouée SANS 
 contournement bug reload/flush (`.tacsuite-prep/replay-recipe-ported.mjs`,
 variante de `replay-recipe.mjs` sans interception de
 `Storage.prototype.setItem`) contre l'app PORTÉE — **succès sans
 contournement**, confirmant bug corrigé côté portage (aucun écrasement
 ...
 **VERDICT PASS** (exit 0), 14/14 pages, **0,00 % de diff pixel sur  14 pages**, **100 % de concordance OCR** titre/fingerprint. (Item non retouché par reprise 2 — reporté tel quel depuis version précédente
 de cette section.)
- **Visuel** : `node tests/visual/compare.mjs oi` : **18/18 PASS**
 ...
 que tuiles MapLibre étaient chargées ou non au moment capture —
 cf. `tests/visual/README.md` § « Chrome au-dessus canvas plein écran »
 pour détail complet et root cause) ; corrigé par démasquage PAR
 BOUTON (au lieu d' rectangle englobant toolbar) + attente explicite de
 l'état `idle` carte OI avant capture. Revérifié **8 exécutions
 ...
 oi-light` : **18/18 PASS** (0,012-0,052 %). Non-régression confirmée sur 
 gate existant : `node tests/visual/compare.mjs pctac` toujours **20/20
 PASS** (0,000-0,072 %, correctif de masque `cartography-modal` est
 scopé à clé `oi` de `APP_CONFIG`, sans effet sur `pctac`).

Légende statuts : **VERT** (E2E vert) · **PARTIEL** (couvert en partie,
nuance dans colonne Résultat) · **NON COUVERT** (aucun test dédié, assumé
explicitement) · **REVUE DE CODE** (vérifié par lecture code câblé, pas
par assertion E2E) · **HORS-SCOPE E2E** (testé unitairement ou nécessitant 
...
| 3 | Navigation : contrôle de cohérence auto à dernière étape | VERT | E2E | `Navigation — contrôle de cohérence déclenché automatiquement…` : `#coherence_alerts_container` peuplé en atteignant l'étape 8 |
...
| 10 | Étape 6 Articulation : 3 listes réordonnables (souris + tactile), blocs MOICP/ZMSPCP/Effraction, sync PATRACDVR | VERT (souris) ; NON COUVERT (tactile) | E2E + unitaire + PDF | `Articulation — blocs MOICP/ZMSPCP/Effraction (création manuelle) + suppression` + `Articulation — rame VL réordonnable par glisser-déposer SOURIS…`. Tactile non testé : `chromium-mobile` (`playwright.config.ts`) n'active pas `hasTouch:true` — ajouter 3e projet dédié serait nécessaire, non fait dans cette passe (assumé, cf. §2). Champ « place chef » (blocs MOICP/ZMSPCP) exercé par recette PDF rejouée (`.tacsuite-prep/oi-reference/recipe-data.json` : `place_chef` sur `moicp_blocks[0]` et vide sur `zmspcp_blocks[0]`, restitué en page PDF via `pdf-engine-v2.ts:1042/1078/1107`). Modale « outils d'effraction » (`openEffractionToolsModal`/`saveEffractionTools`) couverte par test unitaire dédié `tests/unit/oi/oi-articulation.test.ts:448` (ouverture, pré-cochage outils déjà sélectionnés depuis `dataset.tools`, sauvegarde) |
...
| 15 | Étape 7 : menu contextuel clic-droit (cloner/supprimer membre) | VERT | E2E | `PATRACDVR — menu contextuel (cloner/supprimer un membre)` |
...
| 17 | Étape 7 : génération PDF PATRACDVR autonome (pdf-lib) | VERT (déclenchement) | E2E | `PATRACDVR — génération PDF autonome (pdf-lib) déclenche un téléchargement` : déclenchement + fichier non vide vérifiés, pas de comparaison structurelle dédiée (hors scope de ce test, PDF principal OI l'est via `compare-pdf.mjs`) |
| 18 | Étape 7 : réinitialisation isolée reste données | VERT | E2E | `PATRACDVR — réinitialisation isolée du reste des données (resetPatracdvrUI, confirm natif)` |
...
| 21 | Cartographie OI : pins (menu radial, catégories/icônes) | VERT (1 catégorie sur 5) | E2E | `Cartographie — pin générique (Rassemblement) et dessin (rectangle), persistance synchrone` : pin « Rassemblement » posé via `#oi_carto_btn_ping` → `#oi_carto_ping_modal` → clic carte, persisté dans `cartography.pins` (`localStorage`). 4 autres catégories (membres/cyno/rame VL/VL target) non exercées séparément — mécanique commune, pas retestée par catégorie (cohérent avec nuance équivalente `pctac.spec.ts` item #18) |
| 22 | Cartographie OI : dessin (trait/rectangle/cercle, undo/redo, couleurs) | VERT (rectangle + undo/redo) | E2E | Même test que #21 : rectangle tracé (glisser souris), persisté dans `cartography.shapes`, `Ctrl+Z` retire, `Ctrl+Y` restitue. Trait/cercle et palette de couleurs non ré-exercés isolément (même mécanique de dessin, revue de code) |
| 23 | Cartographie OI : recherche adresse (Nominatim) | NON COUVERT | — | Aucun test dédié dans cette passe (risque de dépendance réseau/flakiness, cf. §2) — assumé explicitement, même famille de limite que `pctac.spec.ts` pour recherche carto |
| 24 | Cartographie OI : capture (PNG ou export direct vers champ photo) | NON COUVERT | — | Aucun test dédié — assumé explicitement (cf. §2) |
...
| 27 | Annotation photo : poignées (déplacement/redimensionnement/rotation), appui long mobile | NON COUVERT | — | Poignées non testées (interaction fine, fragile en E2E) ; appui long mobile nécessite contexte tactile (`hasTouch:true`), même limite que #10/#12 — assumé explicitement |
| 28 | Annotation photo : undo/redo (Ctrl+Z/Y), cohérence rendu interactif/export | VERT (undo/redo) ; HORS-SCOPE E2E (cohérence export) | E2E + unitaire | Undo/redo couverts par test #26 ; cohérence pixel-perfect entre rendu interactif et rendu export (`drawAnnotationOnContext`) relève d' test unitaire par hash (mandaté par `SPEC-OI-CONVERSION.md` §11.6), pas de l'E2E |
...
| 30 | Génération : téléchargement PDF (`downloadOiPdf`), format A4/16:9, nom `OI_<date>_<trigramme>.pdf` | VERT (déclenchement) ; PASS (structure) | E2E + `compare-pdf.mjs` | `Génération — téléchargement PDF déclenche un download (downloadOiPdf)` (déclenchement + nom de fichier) ; conformité structurelle complète (14 pages, contenu, ordre sections) certifiée séparément par run de référence PDF ci-dessus — **VERDICT PASS** |
...
| 42 | UI transverse : fenêtre de logs debug mobile (`window.openLogs`) | VERT | E2E | `Fenêtre de logs debug mobile (window.openLogs, résidu window §3.13)` : structure DOM confirmée (script inline `4.html:43-111` porté), ex-stub faible brouillon renforcé |
| 43 | UI transverse : a11y — navigation clavier puces d'étape | VERT | E2E | Couvert dans test Navigation (#1) : puces `.wizard-progress-step` cliquables/activables indépendamment |
| 44 | UI transverse : a11y — focus trap modales, cible tactile ≥44px, anti-zoom iOS | NON COUVERT | — | Non testé — aucun précédent dans `pctac.spec.ts` (même corpus), mesure de taille de cible fragile en CI (dépend zoom/DPI) ; assumé explicitement, cohérent avec gate PC-Tac équivalent (items #15/#33 `NON COUVERT`) |
| 45 | (hors §9) Pont `.oi.zip` → PC-Tac | VERT | E2E cross-app | `Pont OI → PC-Tac — un .oi.zip exporté depuis OI est importable dans PC-Tac (#importOiDockBtn)` : export depuis OI, import dans PC-Tac, bout en bout sur deux apps portées |

## Items non couverts ou partiels — synthèse et justification

Aucun points ci-dessous n'est signe de régression constatée : ce sont
 trous de couverture E2E assumés, cohérents avec limites déjà admises
...
- **Tactile (items #10/#12/#27)** : `chromium-mobile` (`playwright.config.ts`)
 réduit seulement viewport, sans `hasTouch:true`. Émuler fiablement  glisser tactile PATRACDVR/Articulation et l'appui long d'annotation
 nécessiterait soit 3e projet Playwright dédié, soit `page.touchscreen.*`
 explicite — non fait dans cette passe pour ne pas modifier
 `playwright.config.ts` hors mandat.
- **Cartographie — recherche/capture (items #23/#24)** et **pins/dessin hors 1 catégorie/1 outil (items #21/#22)** : séquence pin (modale →
 clic carte) et dessin (glisser souris → `localStorage`) SONT couvertes
 pour au moins représentant de chaque famille (résolution 
 TODO-CABLAGE §5 point 8 brouillon `.tacsuite-prep/draft-oi-spec-notes.md`,
 items #21/#22 tableau ci-dessus) ; recherche Nominatim et capture
 PNG restent non testées (dépendance réseau/risque de flakiness, même
 famille de limite que `pctac.spec.ts` côté Plan).
- **Annotation — 4 types sur 5 (item #26)**, poignées de manipulation directe et cohérence rendu interactif/export (item #27/#28) : même moteur
 `AnnotationEngine` pour 5 types (revue de code), poignées jugées trop
 fragiles en E2E, cohérence pixel mieux couverte par test unitaire par
 hash (`SPEC-OI-CONVERSION.md` §11.6) que par de l'E2E.
- **A11y fine (item #44)** : focus trap et cible tactile ≥44px non testés — aucun précédent de style dans `pctac.spec.ts`, laissé de côté par
 cohérence avec corpus existant plutôt que par oubli.
- **Photos étape 2/3 (items #6/#7)** : non ré-exercées séparément à ces étapes (mêmes inputs que l'étape 5, `medias.ts`), mais flux
 upload+compression+IndexedDB+preview est exercé et confirmé fonctionnel
 par rejeu recette PDF de certification (`J.png`, upload réel).

## Écart DOM documenté hors §9

`#portalLink` (item #40) est ajout P3B.C absent de l'ORIGINAL (aucun
portail n'existait) — cf. `docs/DECISIONS-DOM-ECARTS.md` §6. Son impact sur
 comparaison visuelle (recentrage de `#dockMenu`, barre flex centrée
SANS `collapsed` côté OI, contrairement à PC-Tac) a nécessité masque
élargi à bounding box entière dock plutôt qu'au seul rectangle 
