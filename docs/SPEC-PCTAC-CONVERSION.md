# SPEC-PCTAC-CONVERSION — conversion modules PC-Tac (P2.A0)
...
> Compagnon : `docs/SPEC-PLANMAP-SPLIT.md` (découpage monolithe `planMap.js`).
...
**229 tests unitaires sont déjà verts sur ces modules : ne rien y toucher**, à seule exception documentée en §7 (normalisation `opts` de `ui-platform.ts`).

### 1.3 Modules EXCLUS portage (code mort prouvé — `SPEC-CONTRATS.md` §4)

| Source | Motif | Conséquence obligatoire |
|---|---|---|
| `modules/pctac/dashboard.js` (1232) | jamais importé (`main.js:9-10`, commentaire explicite) | **supprimer** branche `viewId === 'view-dashboard'` de `ui.ts` (`ui.js:110-113`) et section 7 de `pdf-export.ts` (`pdfExport.js:503-538`) — `window.Dashboard` n'existe pas dans `global.d.ts`, ces branches ne compileraient pas |
| `modules/pctac/qrSync.js` (193) | jamais importé, `<script>` absent, DOM cible absent | ne pas porter |
| `modules/pctac/collectionManagers.js` (51) | 0 occurrence hors fichier | ne pas porter |
| `modules/shared.js` | 0 consommateur | ne pas porter |

 **code mort INTERNE à `planMap.js`** (cluster « transform », 10 méthodes) esttraité à part : porté verbatim dans `planmap/legacy.ts`, cf.
...
**Gardes « lib absente ».** tests `typeof X === 'undefined'` deviennent
inatteignables avec import statique. Règle : ** branchement (alerte,
message DOM, `return null`) est CONSERVÉ mot pour mot**, condition est
...
### 1.5 Récapitulatif chemins cibles (aucun chevauchement)
...
Tests : `tests/unit/pctac/<id-de-paquet>.test.ts` ( fichier par paquet, jamais
partagé).
...
1. **Alias obligatoires** (déjà configurés dans `tsconfig.json` **et** `vite.config.ts`, donc valides en build ET en Vitest) :
 `@shared/*` → `src/shared/*` · `@pctac/*` → `src/apps/pctac/*` · `@oi/*` → `src/apps/oi/*`.
2. **Extension `.js` obligatoire** dans tous specifiers relatifs et alias
 (`moduleResolution: "bundler"` + `isolatedModules`) :
 `import { Storage } from '@pctac/storage.js';` — c'est convention déjà
...
4. **Pas de chemins relatifs entre dossiers** : `@pctac/config.js`, jamais
 `../config.js`. **Exception** : à l'INTÉRIEUR de `planmap/`, imports entre
...
5. **`any` interdit** (règle P1.A0, `SPEC-CONTRATS.md` §0.1). Utiliser `unknown`
 + garde, ou type précis. Chaque `unknown` porte commentaire de
 justification.
6. **`!` (non-null assertion) interdit.** Utiliser garde explicite ou 
...
## 3. Stratégie de remplacement `onclick` inline — **DÉCISION**

### 3.1 État lieux (relevé exhaustif)

| Origine | Nombre | Détail |
|---|---:|---|
| `onclick` STATIQUES HTML | **5** | `pctac/index.html:599, 665, 697, 728, 785` (repris verbatim de `pctac2.html:2157, 2223, 2255, 2286, 2343`) : `UI.hideCreatePaxModal()`, `UI.hideEditModal()`, `UI.hideEditAdversaryModal()`, `UI.hideEditHostageModal()`, `UI.closeLightbox()` |
| Handlers GÉNÉRÉS en `innerHTML` par `ui.js` | **13** | `ui.js:214, 217, 460, 461, 490, 491, 508, 530, 545, 550, 551, 557, 563` + 4 attributs `ondragstart/ondragover/ondrop/ondragend` carte photo (`ui.js:544`) |
| `<script>` inline de `pctac2.html` | **1 bloc, 9 lignes** (`:8-16`) | `window.onerror` / `unhandledrejection` → `console` |

> ⚠ **Correction factuelle.** chiffre de « ~350 lignes d'inline » brief
...
> catégorie « scripts retirés » protocole §4.1) : il doit être **réintroduit
> à l'identique en TÊTE de `src/apps/pctac/main.ts`** (cf. §5, étape 0) —> à défaut, journalisation d'erreurs globale de l'app est perdue.
> `docs/DECISIONS-DOM-ECARTS.md` sera complété par gate P2.E.

### 3.2 Décision : **délégation par `data-action`, en P2.D uniquement**

| Question | Décision |
|---|---|
| Mécanisme | ** unique listener `click` en délégation sur `document`**, dispatchant sur `event.target.closest('[data-action]')` ; table `action → handler` vit dans `main.ts`. second listener délégué `change` pour `<select>` de statut photo. |
| Pourquoi pas « par id » | 13 handlers générés sont **paramétrés** (`deleteCollectionItem('pcTacAdversaries','<id>','view-adversaires')`). L'approche par id imposerait de recâbler après chaque `innerHTML` (fuite de listeners, oubli garanti). `data-action` + `data-*` transporte paramètres **dans markup**, sans recâblage. |
| Pourquoi pas `addEventListener` par élément après chaque rendu | `ui.js` reconstruit `innerHTML` à chaque `render*()` : recâblage serait à faire dans 6 méthodes, avec risque d'oubli et de double-binding. délégation est posée ** fois**. |
| Format | `data-action="<verbe-kebab>"` + paramètres en `data-*` : `data-id`, `data-key`, `data-view`, `data-category`, `data-src`, `data-title`, `data-status`. Exemple : `<button data-action="delete-collection-item" data-key="pcTacAdversaries" data-id="…" data-view="view-adversaires">`. || Périmètre | **5 statiques** + **13 générés** + 4 attributs drag carte photo. |
| **Calendrier** | **P2.D**, PAS dans paquets de conversion. En P2.B/P2.C, `ui.ts` génère l'`innerHTML` **VERBATIM avec `onclick`**, et façades `window.UI` / `window.openEditModal` / `window.deleteLogEntry` / `window.deleteCollectionItem` sont **maintenues**. Motif : découpler conversion (risque typage) recâblage (risque comportemental) ; chacun a son gate. |
| Ordre de retrait façades | Exactement `SPEC-CONTRATS.md` §5. 5 binds sans consommateur (`setPaxMode`, `switchMainView`, `toggleSearchMode`, `closeSearchMode`, `filterLogs`) peuvent partir dès P2.D ; `window.UI`, `openEditModal`, `deleteLogEntry`, `deleteCollectionItem` partent **après** délégation. |
| Attributs HTML statique | 5 `onclick` de `pctac/index.html` seront remplacés par `data-action` en P2.D. **Écart DOM** à inscrire dans `docs/DECISIONS-DOM-ECARTS.md` à ce moment-là (il n'est PAS couvert par document actuel). |
...
**Règle générale (fidélité) : chaque module TS pose ses propres globales AU SCOPEMODULE, exactement là où l'original fait** (colonne « `window.*` posé » de
§1.1). Ne PAS déplacer ces affectations dans `main.ts`.

*Justification opposable* : dans graphe ESM, corps modules importéss'exécutent **avant** celui de l'entrée. Déplacer affectations dans `main.ts`
change l'ordre de disponibilité globales — or `tchap-live.ts` s'auto-câble à
l'import et lit `window.PlanMap`. pose au scope module reproduit l'ordre de`main.js:1-12` à l'identique.

*Exception* : `window.deleteLogEntry` et `window.deleteCollectionItem` restent
définies **dans `main.ts`**, à l'intérieur handler `DOMContentLoaded`, comme
`main.js:293` et `:298`.

*Typage* : `global.d.ts` déclare déjà ces propriétés (P1.A0). Aucune modification
n'est requise, **sauf** si agent constate écart de signature — auquel cas
...
`main.ts` n'est PAS dans paquets de conversion : il est produit en P2.D.
 séquence ci-dessous est **imposée** et sera vérifiée au gate P2.E.
...
> désactiverait silencieusement tutoriel.

### 5.2 Imports applicatifs — **ordre de `main.js:1-12`, à respecter à ligne près**
...
**`planmap` AVANT `tchap-live`** : non négociable (cf.`SPEC-PLANMAP-SPLIT.md` §6.1). `dashboard.js` reste non importé, commentaire
`main.js:9-10` est reporté tel quel.

### 5.3 Corps `DOMContentLoaded` — 22 étapes, ordre de `main.js:18-546`

| # | L. `main.js` | Étape |
|---:|---:|---|
| 1 | 20-22 | Enregistrement Service Worker, **gardé** par `location.protocol.startsWith('http')` (⚠ SW est reconstruit en **P4.A** : en P2.D, garder l'appel mais pointer sur futur SW build ou neutraliser explicitement avec `TODO P4.A` — décision au gate) |
...
| 10 | 66-69 | restauration `lastView` + repli si l'id n'existe plus || 11 | 72-76 | restauration thème (`theme`) |
| 12 | 81-104 | soumission `#log-form` |
| 13 | 107-127 | `#confirmCreatePaxBtn` (unicité de couleur) |
| 14 | 130-239 | 3 formulaires de collection (adv/otage/ami) + copies « _sync » vers Photos + statut otage déduit blessures |
| 15 | 242-263 | `change` sur `#adv_photo` / `#hostage_photo` → `Utils.compressImage(…, 800, 800, 0.7)` + miniature || 16 | 266-290 | soumission `#photo-form` (`compressImage(…, 1024, 1024, 0.7)`) |
| 17 | 293-343 | `window.deleteLogEntry` + `window.deleteCollectionItem` (dont purge `pcTacDashboard` via `Persist`, à **conserver** : clé voyage encore dans l'archive) |
...
**Aucune clé localStorage / IndexedDB ne change.** liste de`recon-pctac.md` §5 fait foi.

**Exception documentée et MAINTENUE** : accès `localStorage` directs (hors
`Persist`) sont **conservés à l'identique** — ce sont préférences d'UI ou 
...
| `storage.js` | 14 clés de `clearAllData()` (`removeItem`) | 84-109 |
| `archive.js` | snapshot/restauration brute de l'archive | 62, 169, 179, 193 |
| `imageStore.js` | flag `pcTacIdbMigratedV1` + relecture/réécriture 3 collections | 96, 118, 128 |
Ne PAS « uniformiser vers `Persist` » : cela changerait comportement sur quota
...
## 7. Normalisation actée — 5 fonctions `ui-platform.ts` à défaut TS
 ...
 **paramètre par défaut** TS/JS ne s'applique qu'à `undefined`. L'originalutilise `opts = opts || {}`, qui absorbe **aussi `null`** (et `0`, `''`, `false`).
 appelant passant explicitement `null` — pattern courant en JS legacy, et
autorisé par contrat `UIPlatformContract` puisque `opts?: T` avec
`exactOptionalPropertyTypes` n'interdit pas `null` venant de code non typé
(HTML inline, futur consommateur OI) — provoquerait `TypeError` dans port
alors qu'il fonctionnait dans l'original.

### 7.2 Décision (actée par ce document)

 **5 fonctions** sont normalisées pour **tolérer `null` exactement comme
l'original** :
...
Règles :
1. Signature : `opts?: T | null` ( `?` conserve compatibilité appels
 existants ; `| null` restaure tolérance de l'original).
2. Corps : `const o = opts ?? {};` en **première ligne**, puis remplacer 
 `opts.` par `o.` — **aucune autre modification**.
3. `src/shared/types/contracts.ts` : 5 signatures correspondantes
 (`UIPlatformContract.onLongPress/onDoubleTap/sortable/makeDialog/makeTablist`)
 passent à `opts?: T | null`. C'est **élargissement** : aucun appelant existant ne casse.
4. 229 tests existants doivent rester verts **sans modification** ; ajouter 5 cas `…(el, cb, null)` ne jette pas et applique défauts.

*Écart d'observable* : `?? ` diffère de `||` pour `0`/`''`/`false`. `opts`
valant `0` ou `''` est **impossible** ici ( contrat type en objet) ; 
différence est donc inobservable. Documenté pour revue.

---

## 8. Règles de travail communes à TOUS paquets P2.B / P2.C

Ces règles sont **reprises intégralement** dans chaque instruction de paquet.

### 8.1 Interdits absolus

1. **Aucune écriture dans `/home/nico/Bureau/Web/GStart-main`** (ni fichier, ni `sed -i`, ni `git`, ni fichier temporaire). Lecture seule stricte : `cat`,
 `sed -n`, `grep`, `Read`.
2. Ne toucher qu'aux fichiers listés dans `targets` + son propre fichier de test. **Ne jamais modifier** `src/shared/**` (hors paquet dédié), `tsconfig.json`,
 `vite.config.ts`, `vitest.config.ts`, `package.json`, `pctac/index.html`,
 `styles/*`, ni cibles d' autre paquet.
3. `any`, `!` (non-null assertion), `@ts-ignore`, `eslint-disable` : **interdits**.
4. Aucun refactor code adjacent, aucun renommage de variable, aucune « amélioration » non demandée. **Fidélité avant élégance** (`PLAN.md` §4.7).

### 8.2 Vérifications en contexte parallèle

Plusieurs agents écrivent simultanément dans `src/apps/pctac/`. `npm runtypecheck` compile **tout** projet et remontera donc erreurs venant de
...
> ```
> erreurs situées **hors** de tes fichiers sont ignorées et **signalées** dans
> ton compte-rendu final (elles seront traitées par gate).

### 8.3 Méthode de portage

1. Lire **en entier** fichier source concerné avant d'écrire.2. Porter **méthode par méthode, dans l'ordre fichier d'origine**. Conserver commentaires FR d'origine (ils portent invariants) et **ajouter** 
 référence `// <fichier>.js:<ligne>` en tête de chaque bloc non trivial.
3. Ne convertir que typage. Aucune restructuration de flux de contrôle, aucune extraction de fonction, aucune substitution `for`→`map`.
4. Écrire tests **avant** port pour modules PURS (marqués « TDD » dans l'instruction) ; après port pour modules DOM (tests de fumée ciblés).
...
* **`indexedDB` n'existe PAS sous jsdom** : `image-store.ts` et `tchap-live.ts`
 doivent poser leur propre double (`globalThis.indexedDB = fakeIdb`) dans 
 test, ou tester uniquement branches sans IDB. Ne PAS ajouter de dépendance
 npm.
* **`maplibre-gl` ne peut pas s'instancier sous jsdom** (WebGL absent) : ne
 jamais appeler `new maplibregl.Map(...)` en test ; tester méthodes avec 
 `this` factice portant faux `map`.
...
exécutées + résultats bruts, invariants vérifiés (avec n° de §), écarts
...
|---|---|
| `config.ts` | `normalizeForMatch` (`config.js:159`) fait `.normalize('NFD').replace(/[...]/g, '')` ou classe contient **combinantes U+0300 et U+036F ecrites EN CLAIR** (octets UTF-8 `CC 80` et `CD AF`, verifies a l'`od -c`). **Ecrire forme echappee `/[\u0300-\u036f]/g`** dans TS : strictement equivalent, robuste au copier-coller et au diff. Meme regex en `labelTokens:229` et `matchPhotosByLabel:277,290`. `PIN_ICONS` : 51 entrees, a recopier **verbatim** (ids Material Symbols verifies). |
| `storage.ts` | `saveTpAssociation(label, color)` écrit `assoc[color] = label` — **indexé par COULEUR**, pas par libellé (`storage.js:57`). `saveLogData` **trie en place** (mutation tableau reçu) avant persistance (`:26-29`). `clearAllData` supprime **14 clés**, dont `pcTacPlanLocked` et `pcTacDashboard`. || `utils.ts` | `compressImage` accepte `File` **ou** dataURL (`utils.js:38-46`) ; sortie **toujours** `image/jpeg`. || `log-manager.ts` | `importJson` : `paxMode` est **recalculé d'abord**, puis sert au repli de couleur (`logManager.js:107-112`) — l'ordre est correctif, ne pas l'inverser. Dédup par `id`. Historique lieux LRU **max 30**, insensible à casse. `addEntry` fait `alert()` et retourne `null` : conserver. |
| `image-store.ts` | `put/delete/deleteMany/clear` résolvent sur **`tx.oncomplete`**, pas sur `req.onsuccess` (`imageStore.js:33-42`). `hydrate` ne mute pas liste d'entrée. `migrateFromLocalStorage` est gardée par `pcTacIdbMigratedV1` et pose flag **même si rien n'a migré**. |
| `wheel.ts` | `transform` boutons radiaux **encode leur position** : ne jamais modifier en `:active` (commentaire `wheel.js:330-334`). `_position()` **détruit** roue si l'ancre sort vue + `ext`, et **clampe** sinon (`:137-146`). `_onOutside` ignore 120 premières ms. CSS est injecté seule fois via `#plan-wheel-style` (`:335-351`) — conserver l'injection. `opt.keepOpen` empêche fermeture après action. |
| `archive.ts` | **Validation manifest AVANT toute modification** (`archive.js:129-147`) : refuser sans rien effacer. **Double snapshot** localStorage + images IDB, `rollback()` intégral sur échec (`:160-232`). images sont stockées en `.txt` à l'export mais l'import accepte `.txt` **et** `.bin` (`:218`). Passerelle OI : dédup par `_normName`, photo lue dans `images/<encodeURIComponent(id)>.bin` avec repli non encodé (`:369-370`). |
| `pdf-export.ts` | `PageSizes.A4` est **tuple partagé** : `.slice()` à chaque `addPage` (`pdfExport.js:102-104, 161`). `sanitizeWinAnsi` : table de translittération à recopier **verbatim** (guillemets courbes, tirets, espaces insécables, BOM…). **Supprimer section 7 « BOARD RELATIONNEL »** (`:503-538`, `window.Dashboard` mort). Section 6 : bascule de vue vers `view-plan` + `resize()` + attente 450 ms, restauration dans `finally`. Footer sur **toutes** pages, `void restrictWidth` conservé ou supprimé au choix (variable inutilisée → `noUnusedLocals` : supprimer avec son `void`). |
| `ui.ts` | **Supprimer** branche `view-dashboard` (`ui.js:110-113`). `renderPhotos(filterCategory?)` sans argument reprend `lastPhotoFilter` (`:517-519`). `handlePhotoDrop` : garde `draggedIdx === -1 \|\| targetIdx === -1` (`:598`) — sans elle, `splice(-1,1)` déplace dernière photo. `renderLogTable` : `_logDndBound` évite double binding (`:246-250`) ; `dragend` avec `dropEffect === 'none'` re-render depuis stockage (`:241-243`). `updateTimeInput` lit `window.isTimeInputManuallyChanged`. `esc()` viennent de `@shared/ui-platform.js`. || `tchap-live.ts` | S'auto-câble à l'import (`tchapLive.js:960-961`) — **conserver**. Lit `window.PlanMap` (jamais d'import direct : éviterait cycle mais changerait l'ordre). Deux modes d'auth (token manuel / device-code ProConnect RFC 8628 + refresh). IndexedDB **propre** (`pcTacTchapLive`/`tchapLiveState`), distinct de `pcTacImages`. `fetch('members_config.json')` **relatif** → à réécrire en `/members_config.json` **si et seulement si** l'asset a été copié dans `public/` en P0.A2 ; sinon conserver chemin relatif et **signaler**. Ne PAS confondre avec prototype `tchap-live/` racine (`recon-pctac.md` §7.9). |

---

## 10. Ce qui n'est PAS dans périmètre paquets P2.B/P2.C
...
| Diffs visuels, modernisation `styles/pctac.css`, purge CSS mort (`#version-toggle-btn`) | **P2.F** |
| Service Worker, manifest PWA, précache | **P4.A** |
| Retrait définitif cluster mort `planmap/legacy.ts` | post-P2, sur décision explicite |
