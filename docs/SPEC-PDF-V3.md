...
État : **spécification d'exécution**, arrêtée 2026-08-02.
Décision utilisateur : **A + B**.
Référence opposable : `../../.tacsuite-prep/etude-pdf-strategica.md`.
Structure document : ** 14 sections ACTUELLES de l'OI TacSuite, dans leur ordre actuel**
...
> **Règle de lecture** : * STRUCTURE reste nôtre, LANGAGE VISUEL vient de strategica.*
> Toute divergence de structure (section ajoutée, retirée, renumérotée, réordonnée) est 
> **défaut**, pas amélioration. Y compris défaut hérité **deux sections « 7. »**
...
## 1. Choix bibliothèque — **pdfmake 0.3.11** (retenu)

### 1.1 Banc réel (2026-08-02)

Deux applications Vite 8 (rolldown, mêmes réglages que projet) construites puis
exécutées dans Chromium headless (Playwright projet), même document de test
...
base64. Artefacts banc : `scratchpad/pmvite/` (0.2.23) et `scratchpad/pmvite3/` (0.3.11).

| Critère | pdfmake **0.2.23** | pdfmake **0.3.11** |
|---|---|---|
| dist-tag npm | (ligne de maintenance) | **`latest`** || Bundle Vite brut | 1 741 342 o | **1 387 953 o** |
...
| Enregistrement polices | mutation de globals (`pdfMake.vfs = …`) | **`addVirtualFileSystem()` / `addFonts()`** || Typages | `@types/pdfmake` décrit l'API **0.3** | **`@types/pdfmake@0.3.3` aligné** |

### 1.2 Décision et justification

**pdfmake `0.3.11`** (épinglé exact, comme autres dépendances d'exécution projet)+ **`@types/pdfmake@0.3.3`** en devDependency.
1. C'est version `latest` publiée ( ligne 0.3 n'est plus en beta) ;
2. −226 Ko gzip de bibliothèque par rapport à 0.2.23, à qualité de sortie **identique** ;3. API à promesses : s'insère sans imbrication de callbacks dans `try/catch/finally`
 existant de `downloadOiPdf()` (loader `#pdfLoadingModal`, toasts) ;
4. `addVirtualFileSystem()`/`addFonts()` n'exigent pas de muter globals ;
5. typages DefinitelyTyped correspondent **exactement** à cette API (`addVirtualFileSystem`, `addFonts`, `TCreatedPdf` à promesses) — vérifié en
 compilation sous réglages `strict` + `noUncheckedIndexedAccess` +
 `exactOptionalPropertyTypes` projet, **0 erreur**.

Risque accepté : documentation communautaire (StackOverflow, blogs) est majoritairement
en 0.2.x. **Ne jamais recopier extrait 0.2.x** (`pdfMake.vfs = …`,
...
* `import * as pdfMake from 'pdfmake'` **compile** mais **plante à l'exécution** : `TypeError: Cannot set property fonts of #<en> which has only a getter`
 (l'espace de noms ESM est figé ; `addFonts()` fait `this.fonts = …`). Constaté au banc.
* imports nommés (`import { addFonts } from 'pdfmake'`) sont également proscrits :
 module exporte **instance de classe**, méthodes détachées perdent `this`.
* Chargement : **`await import('pdfmake')`** (import dynamique) dans `engine-v3.ts`, pour
 que bibliothèque forme son propre chunk et ne pèse pas sur démarrage de l'OI.
...
| En-tête de tableau répété à chaque page | `table.headerRows: 1` → en-tête présent sur ** 4** pages d' tableau de 80 lignes |
...
pdfkit n'accepte que **JPEG et PNG**. photos de l'OI viennent d'IndexedDB et peuvent
être en WebP/AVIF/autre (upload utilisateur), ou avoir traversé
`createAnnotatedImageBlob()`. image non supportée fait **échouer tout document**.
⇒ `engine-v3.ts` normalise **avant** de construire définition
(`normalizePhotos()`, §3.5) : tout ce qui n'est pas `data:image/jpeg` ou `data:image/png`
est ré-encodé en JPEG via `<canvas>`, et toute image dont plus grand côté dépasse
`MAX_PHOTO_PX` (2000) est réduite. photo qui ne peut pas être normalisée est
**omise avec avertissement**, jamais propagée telle quelle.
---

## 2. Architecture cible

Nouveau dossier **`src/apps/oi/pdf/`**. `pdf-engine-v2.ts` **n'est pas découpé** (l'interditd'architecture de `SPEC-OI-CONVERSION.md` §7 porte sur ce fichier ; nouveau moteur est
 module distinct, contrainte ne s'y applique pas).
...
**`fonts.ts`** — aucune dépendance à pdfmake (utilisable par voie B seule).```ts
...
```
Note : seule graisse Oswald existe (Medium 500) ; `bold` pointe volontairement sur 
même fichier — titres strategica sont en `font-weight:500`, pas en gras synthétique.

**`theme.ts`** — pur, zéro DOM, zéro pdfmake.```ts
...
```
Contrainte : pdfmake n'a **ni `border-radius` ni `box-shadow`**. « cartes » sont tables 1×1 à bordure 1 pt (`LAYOUT_BORDERED`) ou à liseré gauche épais
(`accentCard`, équivalent `.accent-card`/`.danger-card`/`.warning-card`). « pilules »
sont tables 1×1 bordées ; rangées de pilules sont **découpées en lignes de
`perRow` éléments** (pdfmake ne sait pas faire retourner ligne de `columns`).

**`document-builder.ts`** — pur (aucun accès DOM ni IndexedDB).```ts
...
**`print-style.ts`** / **`print-view.ts`** — voie B.```ts
...
`p.*` = champ de palette. Toutes mesures en mm sont converties par `mm()`.
...
| T2b | `body{background:${p.bg}}` `:649` | `background: (currentPage, pageSize) => ({ canvas: [{ type:'rect', x:0, y:0, w:pageSize.width, h:pageSize.height, color:p.bg, lineWidth:0 }] })` — clé `background` `TDocumentDefinitions`, fond plein cadre sur chaque page (D1/D2, `pdfv3-design-fix/DEFAUTS.md`) | `body{background:${p.bg}}` |
...
| T5 | `h2` `:672` (barre 2 px sous titre) | `blocks.h2` — texte Oswald 17 pt + `{ canvas: [{ type:'line', lineWidth:2, lineColor:p.accent }] }` | `h2{border-bottom:2px solid accent;page-break-after:avoid}` (verbatim strategica) |
...
| T8 | `.card` `:678-689` (fond, bord, **rayon 16 px, ombre**) | `blocks.card` → table 1×1, `LAYOUT_BORDERED` (1 pt `p.border`), **SANS `fillColor` par défaut** (transparente, comme `.box`) — `opts.fillColor` reste seul moyen d'obtenir fond plein (D8, `pdfv3-design-fix/DEFAUTS.md`). **Rayon et ombre non transposables : abandonnés** (écart assumé E1) | `.box{border:1px solid border;padding:8px;page-break-inside:avoid}` (aucune propriété `background`) |
| T9 | `.label` / `.value` `:709-710` | `blocks.labelValue` → `{ text:[{text:LABEL+' : ',bold:true,color:p.accent}, {text:value}] }`, `preserveLeadingSpaces` pour `pre-wrap` | `.k{font-weight:bold;width:30%;background:cardAlt}` + `<p><strong>…</strong></p>` |
...
| T13 | `.monospaced` `:758` | `font: 'JetBrainsMono'` (police par défaut document — no-op) | idem |
| T14 | `.pdf-footer` / `.footer-card` `:807-813` — **présent sur SEULE dernière page** | `footer: (currentPage, pageCount) => …` — bande centrée `OI - <trigramme> - <unité> - CONFIDENTIEL` + `<n> / <N>`, **sur toutes pages sauf garde** (écart assumé E2) | `@page` + bloc `.op-card`/pied strategica ; pagination via `@page`-counters non fiable → bande répétée en `position:running` non utilisée, pied porté par page finale + mention garde |
| T15 | `.bg-watermark` `:738-742` | `{ image, fit:[contentWidthPt, contentHeightPt], opacity: p.watermarkOpacity, absolutePosition:{x:0,y:0} }` dans contenu pages garde/finale | `.watermark{position:absolute;inset:0}` + `.watermark img{object-fit:contain;opacity:…}` (verbatim `OrderPdfStyle.kt:201-204`) |
| T16 | `_fitPageToBudget()` `:558-603` (scale CSS anti-débordement) | **SUPPRIMÉ** — remplacé par pagination automatique + `adaptivePagePx()`/`patracFontPx()` par section | **SUPPRIMÉ** — `page-break-inside:avoid` + police adaptative inline par page dédiée |
### 3.2 14 sections — mapping bloc par bloc

| # | Section (ordre imposé) | Source | Voie A — pdfmake | Voie B — CSS print |
|---|---|---|---|---|
| **1** | **Page de garde** `ORDRE INITIAL` | `:817-855` | Filigrane (T15) ; carte OP/DATE en `absolutePosition:{x: W-mm(60), y: mm(2)}` (table `LAYOUT_BORDERED`, `fillColor:p.cardAlt`) ; `h1` encadré 4 pt centré ; `grid2` [ `card` « 1. SITUATION GLOBALE » = 2 × `labelValue` (générale, particulière) \| `card` « CIBLES(S) » = liste `{nom en gras accent 1.25em} / {stature + ethnie, muted}` séparée par filets 0,5 pt, ou repli `Aucune cible renseignée.` ] ; `pageBreak:'after'` | `.fullpage` + `.watermark` + `.op-card` + `<h1 style="border:4px solid …">` + `.cible` + `.row/.col` |
| **2** | **Fiches adversaires** `2.<i> FICHE ADVERSAIRE : <nom>` ( page par adversaire) | `:894-970` | Page dédiée, `fontSize: adaptivePagePx(champs de la fiche)`. `columns` [ photo principale `figure(src, [mm(70), mm(MAX_ADV_PORTRAIT_H)])` bordée 2 pt accent (omise si absente) \| colonne : `card` IDENTITÉ (`h3` + `kvTable()` — table `.k` bordée `p.border`, colonne label 30 % grasse `fillColor: p.cardAlt` — 6 lignes Naissance/Profession/Situation familiale/Signalement/Signes particuliers/Substances ; + « Moyens Employés » si `me_list` non vide, D4 `pdfv3-design-fix/DEFAUTS.md`) puis `card` DANGEROSITÉ (`h3` filet **danger**, Armes Connues en `p.danger` gras, Dangerosité/ATCD) ] puis `grid2` [ `card` LOCALISATION (Domicile, Volume/Esprit) \| `card` MOBILITÉ (Véhicules/Plaques, Attitude Attendue) ] | `.adv-page` (+ `.adv-fill` si volume < 1000) `style="font-size:Npx"` ; `.fiche-head` > `.fiche-photo` + `.fiche-id` ; `.card-head` ; table `.k` continue ; `.danger-card` dessous |
...
| **3** | `3. ENVIRONNEMENT ET AMIS` | `:972-996` | `h2` + `grid2` [ `card` (Forces Amies/Concours, Terrain/Météo, Éclairage, Lever soleil) \| `card` (Population/Voisinage, Faune/Animaux, Cadre Juridique) ] + `grid2` [ `card` Accès Principal \| `card` Cheminement Initial ] | page dédiée `.adv-page`, `.row/.col`, `field()` |
...
| **8b** | `Articulation : ZMSPCP - <titre>` | `:1075-1094` | `h2` + `grid2` [ `card` « ZMSPCP » = 5 `labelValue` (`Z zone`, `M mission`, `S secteur`, `P points particuliers`, `C conduite à tenir`) \| `card` « Composition par Cellule » = pour chaque cellule bloc bordé 1 pt accent, `fillColor: p.cardAlt` **plein** (jamais de `fillOpacity`, D5 `pdfv3-design-fix/DEFAUTS.md`) : nom de cellule en 0,7 em gras accent souligné + `pillRow(trigrammes, p, { perRow: 6 })` en **pastille contour** (jamais badge plein, D6 ; `badgeRow` n'a plus d'appelant dans `document-builder.ts` mais reste exporté) ; puis `Place du Chef` ] | `.row/.col` + `.box` + `.pill` ; classes `.cell-group/.cell-name/.cell-members` portées **verbatim** de `pdf-engine-v2.ts:760-762` |
...
| **8f** | `Articulation : EFFRACTION - <titre>` | `:1138-1185` | `h2` + `columns` [ photo de porte `figure(src,[mm(70), mm(EFFRAC_TOP_H)])` bordée 2 pt accent, **avec bandeau d'outils** : `pillRow` `tool-badge` posée en `absolutePosition` sous photo (repli `PORTE` si aucun outil) \| `card` « Caractéristiques Techniques » = grille 2 colonnes (`labelValue` ×6 : Structure, Serrurerie, Environnement, Bâti à Bâti mm, Dormant à Dormant mm, Prof. Linteaux mm) + filet pointillé pleine largeur + (H. Porte, H. Marche) + Prof. Bâti sur 2 colonnes ] + `card` « Hypothèses d'Effraction » = table **4 colonnes** (`Hypothèse` 20 %, `Technique / Moyen` 30 %, `Dégagement` 25 %, `Assaut` 25 %), `headerRows:1`, 1re colonne gras accent, repli `Aucune hypothèse saisie` | `.fiche-head` + `.effrac-specs` (grille 2 col., portée verbatim de `:766-768`) + `<hr>` + `<table>` 4 colonnes || **8g** | Galerie `Effraction : <titre>` | `:1186-1187` | `galleryPages()` sur toutes photos `photo_effrac_<id>` | idem |
| **9** | `8. CONDUITES À TENIR GÉNÉRALES` (**omise** si 3 champs sont vides) | `:1195-1216` | `h2` + `grid2` [ `accentCard('accent')` « CAT Générales » \| `accentCard('danger')` « Conditions de Désengagement (NO-GO) » (texte `p.danger` gras) ] + `accentCard('warning')` « Liaison » | `.accent-card` / `.danger-card` / `.warning-card` (verbatim `OrderPdfStyle.kt:131-136`) |
| **10** | `7. RÉCAPITULATIF PATRACDVR` (+ `(Partie n)` si scindé) | `:1219-1280` | Page(s) dédiée(s), `fontSize: patracFontPx(rowCount)`. ** seule table**, `headerRows: 1` (l'en-tête se répète tout seul → **pagination manuelle `MAX_MEMBERS_PER_PAGE` est supprimée**, écart assumé E3). **Nos 8/9 colonnes conservées** : `VL 7 % / PAX 7 % / CELLULE 10 % / FONCTION 14 % / PPALE 10 % / SEC. 10 % / AFIS 8 % / EQPT+GREN. 28 ou 34 % / [DIR 6 %]` — colonne `DIR` présente **seulement si** au moins membre a `dir` non vide (condition `:1227` conservée). Cellule `VL` : `fillColor: p.headerRow` sur 1re ligne de chaque véhicule, vide ensuite. `EQPT/GREN.` = `[equipement, equipement2, grenades, tenue, gpb].filter(v => v && v !== 'Sans').join(', ')` ou `-` | `.patrac` + `<colgroup>` + `<thead>` (en-tête répété à l'impression) ; `.patrac td{text-align:center;overflow-wrap:normal}` (verbatim `OrderPdfStyle.kt:124-128`) |
...
> ⚠️ ** tableau PATRACDVR de strategica compte 12 colonnes** (`OrderHtml.kt:283-301`).
> Ce n'est **pas** notre schéma. On garde **nos 8/9 colonnes** (structure = nôtre) et on
> n'emprunte à strategica que **traitement visuel** : `colgroup` de largeurs calibrées,
...
Comportement source : ** photo par page**, titre `<h2>` = `<titre> (Photo i/N)`, cadre
bordé, légende (`customTitle` ou `<titre> - Détail`), badges d'outils sous légende.

| Aspect | Voie A — pdfmake | Voie B — CSS print |
|---|---|---|
| Découpage | **2 photos max par page** (langage strategica, `OrderHtmlPhotos.kt:70-82`) ; titre suffixé `(suite)` au-delà 1re page — **écart assumé E4** (1/page → 2/page) | idem, `.photo-page-gallery` |
...
| Légende | ligne centrée `p.accent` gras sous cadre | `.photo-caption` |
...
1. **Section vide ⇒ section omise** (galeries vides, page CAT sans champ, PATRACDVR sans membre, fiche adversaire inexistante).
2. **Repli `-`** pour tout champ vide affiché (`|| '-'`), `'Ras'` pour
 `signes_particuliers`, `Aucune cible renseignée.`, `N/A`, `Aucune hypothèse saisie`.
3. **Ordre photos** conservé (extérieur avant intérieur pour MOICP, PR avant domicile pour logistique, baptême **avant** page ZMSPCP, emplacement AO **après**).
...
8. **Échappement** : `document-builder.ts` produit **texte pur** (pdfmake n'interprète pas
 HTML) — risque XSS théorique de `generateHTML` disparaît voie A. voie B,
 elle, produit HTML : elle **doit** utiliser `esc()`/`nl2br()` (port de
 `OrderPdfStyle.kt:216-218`) sur **toute** valeur issue Store.
...
| Élément | Décision |
|---|---|
| `PDFEngineV2.generateHTML()` | **CONSERVÉ tel quel** — c'est rendu de l'**aperçu HTML in-app** (`openPreview`, `:110-142`) et **mode « Présenter ici »** (`openPresentInPlace`, `:158-184`). Aucune modification. |
| `PDFEngineV2.collectAllData()` / `blobToBase64()` | **CONSERVÉS et RÉUTILISÉS** par `engine-v3.ts` — collecteur unique (photos IndexedDB + fusion annotations + fond personnalisé). |
| `PDFEngineV2.downloadOiPdf()` (`:281-467`) | **REMPLACÉ** dans câblage bouton par `downloadOiPdfV3()`. corps html2canvas+jsPDF est **retiré en phase Intégration**, pas dans paquets. || `PDFEngineV2._fitPageToBudget()` (`:558-603`) | Reste utilisé par `openPreview` (`:137`) → **CONSERVÉ**. |
| `html2canvas` | **NE PAS TOUCHER.** Toujours utilisé par `src/apps/pctac/planmap/capture.ts:17` et `src/apps/oi/carto/capture.ts:47`. retirer casserait PC-Tac et carto OI. || `jspdf` | **Seul consommateur = `src/apps/oi/pdf-engine-v2.ts:34`** (vérifié : `grep -rn jspdf src/` ne renvoie que ce fichier + mock test unitaire). fois corps raster retiré, `jspdf` **sort dépendances** (`npm uninstall jspdf`) — résout vulnérabilité `dompurify` transitive notée en Phase 0. **Retrait = phase Intégration**, après validation nouveau chemin. |
| `pdf-lib` | **NE PAS TOUCHER** — moteur PDF PATRACDVR autonome (`#patracdvrPdfBtn`, E2E `oi.spec.ts:626`). |
| `tests/unit/oi/oi-pdf-engine-v2.test.ts` | `describe` `collectAllData` / `generateHTML` / `_fitPageToBudget` **restent verts**. `describe` `downloadOiPdf` (`:467-550`) devra être réorienté en phase Intégration. |
...
`oi/index.html:39-42` — `#downloadPdfBtn` **inchangé** (id, classe, libellé, icône,position). Seule fonction câblée change (`main.ts:576-577`), en phase Intégration :
`window.downloadOiPdf` → `downloadOiPdfV3`. **Aucun écart DOM.**Contrat E2E préservé : `oi.spec.ts:957-971` (téléchargement automatique, nom `^OI_.*\.pdf$`).
...
* **Écart DOM** : à consigner dans `docs/DECISIONS-DOM-ECARTS.md` en **§12** (nouveau
 ...
 dans `GStart-main/4.html`, justifié par voie B présente spec.
* **Impact `tests/visual/compare.mjs`** : **AUCUN masque requis.** 9 états OI capturés (`compare.mjs:185-196` : `step0-situation` … `step7-finalisation`, `cartography-modal`)
 n'ouvrent jamais `#presentationModal`, qui est `<dialog>` fermé par défaut. bouton
 n'est donc visible dans aucune capture de référence. **À revérifier** par
 `npm run test:visual` après ajout : si diff apparaît sur état OI, il s'agit d'
...
3. injection dans `<iframe>` **même origine**, hors écran (`position:fixed;left:-10000px;width:0;height:0;border:0`), via `srcdoc` ;
...
6. retrait de l'iframe sur `afterprint` **ou** après délai de garde de 60 s.
 `<iframe>` (et non nouvel onglet) évite blocage de pop-up et garantit que 
règles `@page` document imprimé sont bien celles de `printCss()`, sans héritage 
...
| Texte OFL 1.1 Oswald | `node_modules/@fontsource/oswald/LICENSE` (déjà dépendance projet) | `src/apps/oi/pdf/fonts/OFL-Oswald.txt` |
| Texte OFL 1.1 JetBrains Mono | `node_modules/@fontsource/jetbrains-mono/LICENSE` | `src/apps/oi/pdf/fonts/OFL-JetBrainsMono.txt` |

* `src/apps/oi/pdf/fonts/README.md` : provenance exacte, empreintes SHA-256, rappel que 
 deux familles sont sous **SIL Open Font License 1.1** (redistribution autorisée, y compris
 embarquée dans PDF, sans obligation de licence sur document produit), et commande de
 régénération `npm run gen:pdf-fonts`.
* `README.md` (racine) : section **« Polices embarquées »** listant Oswald et JetBrains
 Mono, leur licence OFL 1.1 et chemin textes de licence.
* `.ttf` **ne sont pas servis** : ils ne sont lus que par `scripts/gen-pdf-fonts.mjs`.
 Seul `fonts.generated.ts` (base64) entre dans bundle. Aucun fichier de police
...
Outil : **`tests/pdf/verify-structure.mjs`** — script Node autonome (aucune dépendance npm,appelle binaires poppler déjà validés sur machine : `pdfinfo`, `pdftotext`,
`pdffonts`, `pdfimages`).
...
```
Sortie : ligne `PASS <code> — <libellé>` ou `FAIL <code> — <constat>` par assertion,
puis résumé. Code de sortie `0` si toutes assertions passent, `1` sinon.
...
|---|---|---|
| **A1** | Géométrie | `pdfinfo` : `Pages:` ≥ **12** ; `Page size` identique sur toutes pages (`pdfinfo -f 1 -l <N>`), et égale à **841,89 × 595,28 pts** (`--format=a4`, défaut) ou **958,11 × 539,01 pts** (`--format=16:9`), tolérance ±0,5 pt. |
| **A2** | Texte réel | `pdftotext -layout` renvoie ≥ **1 500** caractères non blancs. *(Sur `oi-reference/reference.pdf`, cette assertion échoue — sortie vide : c'est exactement l'anti-pattern que v3 supprime.)* |
| **A3** | Ordre sections | **15 marqueurs** ci-dessous sont tous présents, et l'index de leur **première** occurrence est **strictement croissant**. Normalisation avant recherche : NFC, espaces consécutifs réduits à , apostrophes typographiques `’`→`'`, tirets `–`/`—` conservés. |
| **A4** | Défaut hérité préservé | Exactement **2** occurrences distinctes d' titre commençant par `7. ` (`7. ARTICULATION…` et `7. RÉCAPITULATIF…`). seule ⇒ FAIL (section perdue ou renumérotée à tort). |
| **A5** | Polices vectorielles | `pdffonts` : ≥ 3 lignes ; **toutes** polices listées ont `emb = yes` **et** `sub = yes` ; au moins famille contenant `Oswald` et contenant `JetBrainsMono`. Aucune police non embarquée tolérée. || **A6** | Absence de rastérisation | `pdfimages -list` : nombre d'images ≤ `--photos` (défaut **0**). FAIL supplémentaire si `nb_images == nb_pages` **et** qu'au moins image par page couvre ≥ 80 % surface de sa page (signature exacte de html2canvas+jsPDF). || **A7** | Poids | Avec `--photos=0` : taille fichier ≤ **1 048 576 o** (1 Mio). *(Référence « avant » : 10,8 Mo pour `GStart-main/OI_2026-04-27_MHX-3.pdf`, 2,53 Mo pour `reference.pdf`.)* || **A8** | Données saisies | Si `--sample=<json>` : chaque chaîne tableau `expect[]` fichier apparaît dans sortie `pdftotext`. fichier d'échantillon de référence est dérivé de `../../.tacsuite-prep/oi-reference/recipe-data.json` (nom d'opération, date, trigramme rédacteur, nom d' adversaire, trigramme PATRACDVR, fragment de `missions_psig`). |

**Liste 15 marqueurs (ordre imposé, verbatim)** — source :`oi-reference/fingerprint.md` §« Ordre sections » + `pdf-engine-v2.ts`, et
identique à liste déjà assertée par `tests/unit/oi/oi-pdf-engine-v2.test.ts:343-359` :
...
Marqueurs **4, 8, 10, 11, 12, 13** conditionnés par données. Mode par défaut
(**strict**) : 15 sont exigés — jeu de rejeu `recipe-data.json` produit tous.
Mode `--lenient` : marqueur conditionnel absent est signalé `SKIP` sans faire échouer,
mais l'ordre marqueurs **présents** reste asserté.

### Ce que protocole **NE** fait **PLUS**

* Pas de comparaison pixel pages : rendu vectoriel est changement **voulu**, l'étalon raster de `oi-reference/reference.pdf` devient **caduc pour voie A**
 (cf. `fingerprint.md`, note « hashes non reproductibles »).
* Pas d'égalité stricte à 14 pages : pagination devient automatique, donc dépendante  volume. Seul **plancher de 12 pages** et l'**ordre sections** font foi.
* diff pixel reste pertinent **uniquement sur images extraites** (`pdfimages`), 
 photos et carto restant raster par nature — non automatisé dans cette version de
 l'outil (constat manuel si besoin).

### Écarts visuels ASSUMÉS (à ne pas traiter comme régressions)

| Réf | Écart | Justification |
|---|---|---|
| **E1** | Perte coins arrondis (16 px) et ombres portées `.card` | pdfmake n'a ni `border-radius` ni `box-shadow` ; strategica utilise cadres nets 1 px |
| **E2** | Bande de pied de page + pagination `n / N` sur **toutes** pages (sauf garde) au lieu seule page finale | pagination automatique rend gratuite et utile ; pied unique était artefact rendu page-par-page |
| **E3** | Suppression pagination manuelle PATRACDVR (`MAX_MEMBERS_PER_PAGE` 12/8) | `headerRows:1` répète l'en-tête et pdfmake coupe au bon endroit ; titre `(Partie n)` disparaît |
| **E4** | Galeries à **2 photos** par page (au lieu d') | Langage strategica (`OrderHtmlPhotos.kt:70-82`), retour créateur « images doubles prennent trop peu d'espace » |
| **E5** | Suppression filigrane sur pages intermédiaires (il n'y en avait déjà pas) et de `_fitPageToBudget` | Pagination automatique |
| **E6** | Palette : accents `#2563eb`/`#dc2626` → **`#0033a0`/`#c0392b`** (clair) et `#3b82f6` → **`#5b9bd5`** (sombre) | Langage visuel strategica (`OrderPdfStyle.kt:30-56`) — c'est l'objet chantier |
| **E7** | Unité de police divergente entre deux voies (points PDF réels côté voie A vs pixels CSS avec plancher côté voie B, cf. §7bis) | Décision orchestrateur (mission BLIND.FIX, point 4) — chaque voie reste fidèle à SON modèle de référence (rendu voie A déjà validé utilisateur ; voie B fidèle au langage strategica en px) |
...
descendaient jusqu'à ~4,7 pt sur fiche à police adaptative minimale
...
serveur, pas unité CSS relative) — cf. `print-style.ts` pour détail
de chaque plancher et sa justification.

**Décision actée, PAS régression à corriger plus tard** : voie A(pdfmake, `document-builder.ts`) exprime ses tailles de police en **points
PDF réels** (`fontPx`/`h3`/`labelValue`… déjà en points, rendu déjà validé
par l'utilisateur lors missions précédentes) ; voie B
...
port quasi verbatim, cf. l'en-tête de `print-style.ts`). deux voies
n'ont donc PAS même unité de référence pour même taille nominale
(1 px CSS ≈ 0,75 pt PDF à 96 ppp) — écart assumé au même titre que E1-E6
ci-dessus, plancher absolu suffit à garantir lisibilité sans
uniformiser deux modèles.
...
| **P2** thème | sonnet | `src/apps/oi/pdf/theme.ts` + test || **P3** primitives | sonnet | `src/apps/oi/pdf/blocks.ts` + test || **P4** constructeur de document | sonnet | `src/apps/oi/pdf/document-builder.ts` + test || **P5** voie B (impression) | sonnet | `src/apps/oi/pdf/print-style.ts`, `print-view.ts` + tests || **P6** moteur v3 | sonnet | `src/apps/oi/pdf/engine-v3.ts` + test || **P7** UI + PWA + écart DOM | haiku | `oi/index.html`, `vite.config.ts`, `docs/DECISIONS-DOM-ECARTS.md` |
| **P8** protocole structurel | sonnet | `tests/pdf/verify-structure.mjs`, `tests/pdf/README.md` |

**Hors paquets — phase Intégration** (orchestrateur) :câblage `#downloadPdfBtn` → `downloadOiPdfV3`, entrée `oiClickActions['print-oi-high-quality']`,
retrait corps raster de `downloadOiPdf()`, `npm uninstall jspdf`, réorientation 
`describe('downloadOiPdf')` test unitaire, ajout tests E2E nouveau bouton,
...
pagination blocs ZMSPCP/MOICP et tableau PATRACDVR de
`document-builder.ts` (voie A, pdfmake), motivé par 3 défauts **prouvés** sur
 PDF réel de 21 pages avant correctif — queues orphelines (« fixer
l'adversaire. » seule sur page), pages à titre seul, mots Store cassés
lettre à lettre dans PATRACDVR (« SHARA N », « GILE TTE », « KODIA Q
...
mots cassés page 15), PASS après ( deux, thème clair ET sombre, format
`a4` ET `16:9`), sans régresser 1628 tests préexistants (1631 après ajout
 3 tests dédiés `tests/unit/oi/pdf/oi-pdf-document-builder.test.ts`).

### Contrainte de départ : module PUR

`document-builder.ts` reste module PUR (zéro pdfmake en VALEUR, cf. son
en-tête) — mesure réelle rendu (`pageSize:{height:Infinity}`, bench
`pdfmake-pagination-bench` q5) est donc **hors de portée** de ce fichier :
c'est capacité seul harnais de test (`tests/pdf/generate-from-fixture.mjs`),
jamais code de production. modèle ci-dessous est donc entièrement
**heuristique par volume de caractères**, dans prolongement direct barèmes déjà en place (`documentFontPx`, `adaptivePagePx`, `patracFontPx`,
`theme.ts`) — pas mesure de rendu réelle.

### 1. Police adaptative AVANT scission (priorité 1-2)

`buildZmspcpPage`/`buildMoicpPage` calculent palier de police propre au
bloc via `adaptivePagePx()` (déjà utilisée par fiche adversaire), sur 
champs cœur (Z/M/S/P ou M/O/I/P, `cat`, `place_chef`) + nombre de lignes
« virtuelles » proportionnel au nombre de groupes de cellule. champ « C
conduite à tenir » volumineux (beaucoup de `\n`) fait donc naturellement
tomber palier au minimum (9 px) **avant** toute décision de scission —
sur `long-case.json` (bloc ZMSPCP à 20 items), ce seul mécanisme suffit à
faire tenir bloc entier sur page (vérifié par rendu réel +
...
aux frontières `\n(?=-\s)` — jamais en milieu de phrase. Sans tiret détecté,
 champ reste bloc unique, intact, JAMAIS scindé (repli identique au
comportement pré-correctif). Chaque item est ensuite rendu comme élément
`unbreakable:true` **individuel** (`dashItemList`) — pas bloc entier :
finding #1 banc (`unbreakable` dépassant page = **suppression
SILENCIEUSE**, 0 ligne rendue, aucune erreur) interdit d'appliquer
`unbreakable` à bloc taille d' page ; chaque item pris seul reste
très en-deçà de cette limite. Ce rendu par item s'applique **dès qu'
frontière existe**, même sur seule page — pas seulement en cas de
scission — pour que pdfmake ne puisse jamais rompre phrase entre deux
lignes wrappées d' même item.

### 3. Scission « (suite) » en dernier recours (priorité 3, garde-fou)

Si nombre d'items dépasse `catItemsPerPageBudget(fontPx)` (`theme.ts` —
...
12/16/20/26 items selon palier 14/12/10/9 px), bloc est scindé enfragments de `budget` items (`chunkItems`). Chaque fragment devient sa PROPRE
page : premier conserve champs cœur + composition par cellule, 
suivants portent titre `<Titre> (SUITE)` (port règle cible
strategica, `h2()` majuscule texte) et uniquement suite items —
jamais de duplication ni de perte (vérifié exhaustivement par test unitaire,
30 items synthétiques → exactement 1 scission à l'item 26/30, chaque itemprésent exactement fois). Convention de saut de page : identique à
`galleryPages()` (cf. en-tête `document-builder.ts`) — seule première page
 bloc reste nue, suivantes portent déjà leur propre `pageBreak:'before'`,
consommées par `pushPages()` (pas `pushPage()`).

### 4. Sections vides omises

Déjà couvert par code préexistant, non touché par ce correctif :
...
aucune page « titre seul » n'est donc générée pour section vide.

### 5. PATRACDVR : largeurs adaptées (`buildPatracPage`)

Cause bug (bench q3, confirmée sur `long-case.json` p.15 AVANT correctif) :
 colonnes « code court » (véhicule, trigramme, cellule, fonction, armes,
AFIS, direction) étaient en largeur **pourcentage fixe** sans `noWrap` — 
mot sans espace plus large que colonne (« SHARAN », « KODIAQ », « GILETTE »)
était cassé **lettre à lettre** par l'algorithme de wrap de pdfmake (aucune
césure au tiret possible pour mot Store). Corrigé en largeurs `'auto'`
+ `noWrap:true` sur cellule pour **toutes** colonnes code court (largeur
= celle plus long libellé RENCONTRÉ, jamais coupée) ; seule EQPT/GREN.
(texte combiné potentiellement long, plusieurs mots) reste en `'*'` sans
`noWrap`, conservant son retour à ligne normal. `patracFontPx()` (palier
9-14 px selon nombre de lignes, `theme.ts`) est inchangé.

### 6. Groupement 2 colonnes (ZMSPCP/MOICP)

Déjà porté par `grid2()` (champs cœur à gauche, composition par cellule à
droite) — inchangé par ce correctif, préservé sur première page
de chaque bloc scindé ( fragments « (SUITE) » n'ont qu' colonne gauche
utile, droite reste espace réservateur `{ text: '' }` pour garder 
mise en page à 2 colonnes cohérente).

### Limites assumées

* budget `catItemsPerPageBudget` est heuristique par NOMBRE D'ITEMS,
 pas par volume de caractères par item — bloc à 20 items très longs
 (chacun sur 3-4 lignes) pourrait en théorie encore déborder d' page
 malgré nombre d'items sous budget ; non observé sur jeux de
