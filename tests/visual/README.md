...
voir "Pourquoi config séparé" ci-dessous)
...
## Pourquoi config Playwright séparé

 `playwright.config.ts` racine cible `testDir: './tests/e2e'` avec
`baseURL: http://127.0.0.1:9678` ( TacSuite en cours de portage). Ces
baselines ciblent l'ORIGINAL (9679) via URLs absolues (`page.goto`
explicite), serveur différent — pas de `baseURL` pertinent ici.

Élargir `testDir` racine à `./tests` aurait aussi fait matcher
`tests/unit/smoke.test.ts` (spec **Vitest**, pattern par défaut Playwright =
`**/*.@(spec|test).?(c|m)[jt]s`) et cassé son exécution sous runner
Playwright. D'où config scope au dossier `tests/visual/`, reprenant à
l'identique 2 projets/viewports config racine (`chromium-desktop`
...
Attente avant chaque capture : 300 ms après clic (mise à jour synchrone 
...
pas "onglets/sections principales") : `quickEditModal`,
...
`importSelectModal`. Ces états relèvent checklists fonctionnellesPlaywright e2e phases suivantes (P3.D), pas baselines visuelles P0.
...
Attente avant chaque capture : 300 ms après clic (toggle synchrone de
classes/style) ; `networkidle` (dégradé, timeout 15 s) + 1.2 s avant
`initial-main-courante` ( carte MapLibre s'initialise dès `main.js`, même
onglet Main Courante actif) et avant `tab-plan` (rechargement tuiles au
`resize()` déclenché par `PlanMap.refresh()`).

Non capturés (hors périmètre — outils secondaires nécessitant 
interaction carte supplémentaire, pas de simple clic d'ouverture de
panneau) : roue contextuelle de ping (`plan_btn_ping`, nécessite point
sur carte), mesure distance/azimut, verrou par-annotation, capture
carto (`plan_btn_capture`), AOI hors-ligne (`plan_btn_aoi`, arme 
cadrage rectangle one-shot — pas d'état visuel distinct au clic seul, et
risque de déclencher vrai téléchargement de tuiles), 2D/3D relief,
...
(`window.PocheTuto`). Ces états relèvent checklists fonctionnelles
Playwright e2e phases suivantes (P2.E), pas baselines visuelles P0.

## Zones à MASQUER dans comparaisons visuelles FUTURES

 baselines de ce dossier NE masquent rien elles-mêmes (première capture
de référence — tuiles telles que chargées au moment run, ainsi que
 boutons BETA d'origine, font partie intégrante PNG). Pour diffs
...
`cartography-modal`) nécessitent masque `canvas.maplibregl-canvas`
ci-dessus, et **toutes autres captures deux apps** (donc 38
baselines sans exception, chaque état contenant l'en-tête) montrent 
bouton BETA (`#version-toggle-btn` côté PC-Tac, `#beta-button` côté OI),
absent porté. Son masque est donc également OBLIGATOIRE, sur 100 % 
états, pas seulement sur états carte.
...
- **PC-Tac** : `#dockMenu` porte `class="dock-menu collapsed"` par défaut, et
 ...
 none; }` (`styles/pctac.css`) masque tous items dock sauf 
 toggle. Aucun 10 états de `compare.mjs pctac` (`APP_CONFIG.pctac`
 ci-dessus) ne clique `#dockToggleBtn` : `#portalLink` reste `display:none`
 dans 20 captures (2 viewports), donc invisible et sans impact sur 
 diff pixel. **Aucun masque ajouté à `HEADER_MASK`/`compare.mjs` — vérifié
 inutile, pas oubli.**
- **OI** : à différence de PC-Tac, `#dockMenu` de `oi/index.html` N'A PAS
 classe `collapsed` par défaut (dock déployé au premier rendu) —
 `#portalLink` y est donc visible dès chargement, sur 9 états / 18
 captures de baseline OI. `compare.mjs oi` n'était PAS gate mission
 P3B.C (seul `compare.mjs pctac` était requis par ce mandat-là) et n'avait
 pas été relancé/corrigé à ce moment.

**Mise à jour (P3B.E, `compare.mjs oi` redevenu gate actif)** : masque
ajouté à `tests/visual/compare.mjs` (`PORTAL_LINK_MASK.oi`). Premier essai
insuffisant : masquer seulement rectangle nouvel icône (mesuré
56×56 desktop / 38×52 mobile) laissait 16/18 états en FAIL (~0,13-0,43 %,juste au-dessus seuil 0,1 %) — `#dockMenu` est barre flex CENTRÉE
sans `collapsed` côté OI, donc insérer item RECENTRE toute 
barre et translate tous icônes voisins d' largeur de slot, pas
seulement `#portalLink` lui-même. masque couvre donc désormais 
bounding box entière de `#dockMenu` (mesurée en direct sur porté) :
...
Aucun impact sur `compare.mjs pctac` (masque scopé à clé `oi` de
`PORTAL_LINK_MASK`, absente pour `pctac`) — revérifié **20/20 PASS**.

### Chrome au-dessus canvas plein écran (`cartography-modal`, P3B.FIX)

`#cartographyModal` est `<dialog>` `100vw`/`100vh` (4.html:3613-3620,
verbatim) : `canvas.maplibregl-canvas` y occupe donc totalité 
viewport, sans aucun chrome de PAGE visible autour (contrairement aux états
carte de PC-Tac). seule zone de chrome réelle est `.oi-carto-toolbar`
(colonne de 8 FABs circulaires `.oi-carto-fab`, ⌀50px chacun), qui flotte
par-dessus canvas.

**Reprise 1 (BLOQUANT R3)** démasquait rectangle ENGLOBANT de`.oi-carto-toolbar` (50×456) pour lui redonner statut d'assertion réelle
plutôt que de noyer dans masque canvas — mais colonne de disques
séparés par interstices TRANSPARENTS laisse canvas visible à travers
ces interstices, ce qui réintroduit non-déterminisme (tuiles réseau) que
 masque canvas existe pour supprimer. Mesuré sur 8 exécutions dépôt :
2 régimes discrets sur `cartography-modal-desktop` — 332px (tuiles déjà
chargées au clic) / 8036px = bbox entière toolbar (tuiles pas encore
chargées), `run()` n'attendant qu' `waitForTimeout(500)` fixe sans
attendre fin de chargement tuiles.

**Reprise 2 (BLOQUANT R3bis)**, deux correctifs combinés :1. `unmaskSelector: '.oi-carto-toolbar .oi-carto-fab'` — rect **par
 bouton** (8 × 50×50) plutôt qu' seul rect englobant. interstices
 retombent dans masque canvas (non comparés) ; `paintMask()` accepte
 désormais tableau d'exclusions (`excludeRects`) au lieu d' rect
 unique.
2. `state.waitForMapIdle: true` — après clic, `captureState()` attend
 `window.OICarto.map.loaded() && .areTilesLoaded()` (API publique
 maplibre-gl, filet de sécurité 5s) avant capture, en plus de
 `networkidle` (qui ne couvre que requêtes réseau, pas rendu
 interne MapLibre).
...
1 296 000 px, mesuré) au lieu de ~98,24 % avant — toujours sous seuil.
**Résidu 332px → 220px, root cause (MOYEN)** : résidu documenté enreprise 1 (anneau de focus bleu sur 1er bouton, `#oi_carto_btn_search`,
absent baseline) N'EST PAS écart DOM/CSS portage — vérifié par
...
chargée par deux apps) entre l'ORIGINAL (9679) et porté (9678) à
séquence d'interaction égale. divergence est artefact de
**méthodologie de capture** : heuristique `:focus-visible` de Chromium
dépend de l'historique d'interaction page — `capture-baseline.spec.ts`
parcourt 7 étapes wizard par clics réels AVANT d'ouvrir 
cartographie (session continue), tandis que `compare.mjs` fait 
`page.goto` frais puis clic immédiat sur `#cartographyBtn` PAR ÉTAT (états
...
indicateur visuel) — DEUX comportements sont donc légitimes et
reproductibles sur l'original lui-même, purement fonction chemin
emprunté par harnais de test, pas d' régression produit. **Pas
d'entrée `DECISIONS-DOM-ECARTS.md`** en conséquence (réservé aux écarts DOMréels type `#portalLink`/`#tl-orbat-style`) : il n'y a rien à corriger côté
application, masquage par bouton (ci-dessus) réduit mécaniquement 
résidu (332→220px) en excluant majorité halo (`outline`/`box-shadow`
dessinés hors boîte de bordure bouton, donc hors rect
50×50 démasqué) sans avoir besoin de neutraliser explicitement ; reliquat (bord de l'arc 1er bouton + quelques pixels d'antialiasing
d'icône sur boutons 2-4) reste largement sous seuil de 0,1 %.
...
`tests/visual/compare.mjs` définit clé d'app dédiée `oi-light`
(`APP_CONFIG['oi-light']`, mêmes `states`/`entryUrl` que `oi`, ajoute
`theme: 'light'`) : `captureState()` bascule porté en mode clair par 
clic réel sur `#darkModeToggle` (même mécanisme que
`capture-oi-light.mjs`), avec garde nécessaire ici : contrairement à
`capture-oi-light.mjs` ( seule navigation par viewport, thème basculé 
fois puis navigation SPA entre états), `compare.mjs` fait `page.goto` par
ÉTAT — dès 2e état, `localStorage.theme` (persistée par
`handleThemeToggle()` au 1er clic) fait déjà démarrer page en
`light-mode` ; cliquer à nouveau sans garde ferait REBASCULER en sombre
(bug constaté à l'implémentation : 9/18 états en `ERROR` par timeout,
exactement 1 état sur 2 — parité trahissant l'aller-retour clair/sombre à
...
`oi-light` (mêmes bounding boxes qu'en mode sombre — bascule de thème ne
change que couleurs, aucune propriété de layout, vérifié dans
...
conservée ici uniquement à titre d'exemple piège à ne pas reproduire.

**Piège à éviter** : cette forme, prise telle quelle, ne suffit PAS pour`#version-toggle-btn` / `#beta-button`. Playwright peint masque sur capture page ** screenshotée au moment de l'appel** —
ici page COMPARÉE, c'est-à-dire porté servi sur 9678. Or
`#version-toggle-btn` / `#beta-button` n'existent PLUS dans DOM porté
(c'est précisément l'écart admis) : `page.locator(...)` y résout 0 élément,
donc `mask` n'y peint rien pour ces deux sélecteurs. Et baseline en face
est PNG déjà enregistré sur disque (`tests/visual/baseline/...png`), pas
 page vivante — impossible d'y appliquer `Locator` Playwright au
moment diff. En s'arrêtant à forme ci-dessus, zone bouton BETA
resterait donc intégralement visible (bouton réel) côté baseline et
intégralement vide côté porté → diff garanti à ~100 % pixels de cette
zone sur 38 états, quel que soit seuil retenu.

**Stratégie retenue : masquage par RECTANGLE FIXE**, appliqué identiquementaux DEUX images (baseline ET capture porté) par script de comparaison
de P2.F/P3.D, indépendamment présence sélecteur DOM. Concrètement :
avant `pixelmatch` (ou équivalent), peindre même rectangle (couleur
unie) aux mêmes coordonnées pixel sur deux images — copie en
mémoire baseline, fichier `.png` sur disque restant intact — puis
...
Ces 4 rectangles sont CONSTANTES (pas de bounding box interrogée en
direct), à différence de `canvas.maplibregl-canvas` qui existe deux
côtés (bounding box interrogeable en direct sur porté et réutilisable
telle quelle sur baseline, layout DOM/CSS identique par protocole) : il
n'y a côté porté aucun élément équivalent à `#version-toggle-btn` /
`#beta-button` à partir duquel dériver position. Elles restent valables
tant que mise en page de l'en-tête ne change pas — à revérifier si P2.F
/ P3.E (modernisation CSS) déplacent éléments d'en-tête adjacents.

**Correction (P2.FIX reprise 2)** — ces 4 rectangles sont constantesmesurées à l'état NON défilé (`scrollY=0`), ce qui est cas quasi
totalité états capturés MAIS PAS de tous : côté PC-Tac, l'état
`tab-plan-panneau-tchap-live` (clic sur `#tl_toggle`, ouverture panneau
Tchap live) fait défiler page de ~28-32 px en mobile (390×844) et
~228 px en desktop (1440×900) avant capture finale. Appliquer 
rectangle constant tel quel désaligne masque position réelle 
bouton BETA dans l'image (mesuré directement sur PNG de baseline :
`x=324..369 y=30..57` dans `tab-plan-mobile.png`/`initial-main-courante-mobile.png`
non défilés, mais `x=324..369 y=2..25` dans
`tab-plan-panneau-tchap-live-mobile.png`, défilé) — masque manque
intégralement sa cible et bouton fuit dans diff (mesuré : 393 px de
...
page vivante ( porté, seul capturable en direct) juste avant 
screenshot de chaque état, puis en translatant rectangle constant de
`-scrollY` avant de peindre masque sur DEUX images (baseline
figée ET capture fraîche) — layout DOM/CSS étant identique deux
côtés par protocole zéro régression, même décalage de scroll s'applique
aux deux. coordonnées `x`/`y` tableau ci-dessus restent donc 
constantes de référence (état non défilé) ; c'est uniquement au moment 
diff que `y` est ajusté dynamiquement. Sans effet sur états qui ne
défilent pas (`scrollY=0` → rectangle inchangé).

**Alternative écartée** : re-capturer 38 baselines avec boutonsBETA neutralisés (ex. `display:none` injecté) dans l'original. Rejetée
pour ne pas invalider/refaire 38 captures déjà produites et validées
(gate P0.A6), et parce que neutralisation via CSS provoquerait 
reflow de l'en-tête sur l'original ( éléments voisins bouton BETA
se repositionneraient), ce qui changerait baseline au-delà seule
...
captures produites, écarts éventuels). `dialog` natif (`alert()`) peut en
théorie apparaître sur `cartography-modal` si `maplibregl` échoue à se
charger depuis CDN unpkg (`oi_cartographie.js` : *"Librairie
cartographique indisponible (réseau ?)"*) — test l'intercepte et 
journalise (`console.warn`) sans faire échouer capture, l'état
`cartography-modal` serait alors modale vide/dégradée plutôt qu' échec
de test.
