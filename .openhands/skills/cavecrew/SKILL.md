...
description: >
 ...
 work inline or using vanilla `Explore`. Subagent output is caveman-compressed so the tool-result injected back into main context is ~60% smaller — main
...
**Locate → fix → verify** (most common):1. `cavecrew-investigator` returns site list.
